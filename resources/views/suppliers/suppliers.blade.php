...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="px-4 py-4 bg-white shadow-sm sm:px-6 sm:py-6">
        <div class="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
            <!-- Title and subtitle -->
            <div>
                <h1 class="text-2xl font-light text-gray-800 sm:text-3xl">Suppliers Management</h1>
                <p class="text-sm text-gray-500 sm:text-base">Manage supplier information and data</p>
            </div>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="text-sm font-medium text-gray-500 ms-1 md:ms-2">Suppliers</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <div class="grid grid-cols-1 gap-6 p-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        ...
        <a href="{{ asset('suppliers/addSupplier')}}" class="group">
            <div class="h-full p-6 transition-all duration-300 transform bg-[{{ $settings[7]->value }}] shadow-sm rounded-xl hover:shadow-md hover:-translate-y-1">
                <div class="flex flex-col items-center justify-center h-full text-center">
                    <div class="p-4 mb-4 transition-colors duration-300 rounded-full bg-gradient-to-br from-blue-500 to-blue-700">
                        <img src="{{ asset('images/suppliers/addNewSuppliers.png') }}" alt="Add Supplier" class="w-8 h-8">
                    </div>
                    <h3 class="text-lg font-medium text-white">Add New Supplier</h3>
                    <p class="mt-1 text-sm text-gray-300">Create new supplier records</p>
                </div>
            </div>
        </a>
        @endif
        ...
        <a href="{{ asset('suppliers/supplierList')}}" class="group">
            <div class="h-full p-6 transition-all duration-300 transform bg-[{{ $settings[7]->value }}] shadow-sm rounded-xl hover:shadow-md hover:-translate-y-1">
                <div class="flex flex-col items-center justify-center h-full text-center">
                    <div class="p-4 mb-4 transition-colors duration-300 rounded-full bg-gradient-to-br from-green-500 to-green-700">
                        <img src="{{ asset('images/suppliers/supplier List.png') }}" alt="Supplier List" class="w-8 h-8">
                    </div>
                    <h3 class="text-lg font-medium text-white">Suppliers List</h3>
                    <p class="mt-1 text-sm text-gray-300">View and manage all suppliers</p>
                </div>
            </div>
        </a>
        @endif
        ...
        <a href="{{ asset('suppliers/importSupplier')}}" class="group">
            <div class="h-full p-6 transition-all duration-300 transform bg-[{{ $settings[7]->value }}] shadow-sm rounded-xl hover:shadow-md hover:-translate-y-1 hidden">
                <div class="flex flex-col items-center justify-center h-full text-center">
                    ...
                    <h3 class="text-lg font-medium text-white">Import Suppliers</h3>
                    <p class="mt-1 text-sm text-gray-300">Bulk import supplier data</p>
                </div>
            </div>
        </a>
        @endif
