                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <p class="text-sm font-medium text-gray-700 ms-1 md:ms-2">Import Suppliers</p>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <table id="impTable" class="w-full text-sm text-left text-gray-500 rtl:text-right">
                    <thead class="text-xs text-white uppercase bg-[{{ $settings[7]->value}}]">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3">Suppliers Code</th>
                            <th scope="col" class="px-6 py-3">Suppliers Name</th>
...
document.getElementById('addDataBtn').addEventListener('click', () => {
    ...
    rows.forEach(row => {
        ...
        const supplier = {
            supplier_code: columns[1].textContent.trim(),
            supplier_name: columns[2].textContent.trim(),
            mobile_number: columns[3].textContent.trim(),
            address: columns[4].textContent.trim(),
            email: columns[5].textContent.trim(),
            user_id: columns[6].textContent.trim(),
            city_id: columns[7].textContent.trim(),
            status_id: columns[8].textContent.trim(),
            city_name: columns[9].textContent.trim(),
        };
        supplierData.push(supplier);
    });

    if (supplierData.length > 0) {
        // Make an AJAX request to send the data to the backend for insertion
        fetch('/import-suppliers', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                alert('There was an error while adding the data.');
            }
        })
        .catch(error => {
