@include('layouts.header')

    <div class="h-[90%] flex flex-col">
        <!--breadcrumbs-->
        <div class="px-12 py-5 max-sm:px-6">
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    <li class="inline-flex items-center">
                        <p class="inline-flex items-center text-sm font-medium text-gray-700">
                            <svg class="w-3 h-3.5 me-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                fill="currentColor" viewBox="0 0 20 20">
                                <path
                                    d="m19.707 9.293-2-2-7-7a1 1 0 0 0-1.414 0l-7 7-2 2a1 1 0 0 0 1.414 1.414L2 10.414V18a2 2 0 0 0 2 2h3a1 1 0 0 0 1-1v-4a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v4a1 1 0 0 0 1 1h3a2 2 0 0 0 2-2v-7.586l.293.293a1 1 0 0 0 1.414-1.414Z" />
                            </svg>
                            Main Panel
                        </p>
                    </li>
                    <li aria-current="page">
                        <div class="flex items-center">
                            <svg class="w-3 h-3 mx-1 text-gray-400 rtl:rotate-180" aria-hidden="true"
                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                    stroke-width="2" d="m1 9 4-4-4-4" />
                            </svg>
                            <p class="text-sm font-medium text-gray-700 ms-1 md:ms-2">
                                Billing
                            </p>
                        </div>
                    </li>
                </ol>
            </nav>
        </div>
        <!--main content-->
        <div class="flex h-full gap-2 px-12 pb-5 overflow-y-auto max-sm:px-6 max-xl:flex-col">
            <!--blue div + textfields + table-->
            <div class="bg-white flex flex-col rounded-lg border-2 border-[#00000096] w-full">
                <div class="flex flex-col">
                    <!--blue div-->
                    <span
                        class="bg-[{{ $settings[7]->value}}] h-20 max-sm:h-fit rounded-t-md flex max-lg:flex-col max-lg:py-1 justify-between px-4 items-center">
                        <span class="flex gap-3 text-white max-sm:text-sm max-sm:w-full">
                            <p>Sales Invoice</p>
                            <p>Branch: Colombo</p>
                        </span>
                        <!--
                        <span class="flex justify-end gap-3 max-sm:p-2 max-sm:text-sm max-sm:flex-col max-sm:w-full">
                            <button data-modal-target="default-modal" data-modal-toggle="default-modal"
                                class="px-6 py-1 text-black bg-white border-2 rounded-lg">
                                View Hold List
                            </button>
                            <button class="px-6 py-1 text-black bg-white border-2 rounded-lg">
                                Retail
                            </button>
                            <button class="px-6 py-1 text-black bg-white border-2 rounded-lg">
                                Wholesale
                            </button>
                        </span>
                        -->
                    </span>
                    <!--textfields-->
                    <div class="flex gap-3 p-2 h-fit max-lg:flex-col max-lg:items-center">
                        <!--custom 
                        <div class="custom-select sm:w-fit">
                            <select id="customer" name="customer"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 hidden">
                                <option value="">Walk-in customer</option>
                                <option value="Customer 1">Customer 1</option>
                                <option value="Customer 2">Customer 2</option>
                                <option value="Customer 3">Customer 3</option>
                                <option value="Customer 4">Customer 4</option>
                            </select>
                        </div>
                        combobox-->
                        <!--add customer btn (blue
                        <button type="button" data-modal-target="customer-modal" data-modal-toggle="customer-modal"
                            class="text-white w-fit bg-[#1b4f72] hover:bg-[#1b4f72]/90 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                class="bi bi-person-circle" viewBox="0 0 16 16">
                                <path d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0" />
                                <path fill-rule="evenodd"
                                    d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8m8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1" />
                            </svg>
                        </button>
                        )-->
                        <!--search using IMEI-->
                        <form method="GET" action="{{ route('mobilebilling') }}">
                            <div class="flex gap-3 max-sm:w-full max-lg:flex-col max-lg:w-full">
                                <div class="flex w-full gap-3 max-md:flex-col">
                                    <input type="text" name="imei"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                        placeholder="Search using IMEI" value="{{ request()->imei }}" />

                                    <div class="flex w-full custom-select">
                                        <input type="text" name="brand" 
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" 
                                            placeholder="Search using Brand" value="{{ request()->brand }}" />
                                    </div>

                                    <div class="flex w-full custom-select">
                                        <input type="text" name="model" 
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" 
                                            placeholder="Search using Model" value="{{ request()->model }}" />
                                    </div>
                                    
                                    <button type="submit" class="py-3 px-6 bg-[{{ $settings[7]->value}}] text-white rounded-lg max-md:flex justify-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16">
                                            <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001q.044.06.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1 1 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0" />
                                        </svg>
                                    </button>
                                </div>
                            </div>
                        </form>


                    </div>
                </div>
                <!--table from flowbite-->
                <div class="relative h-full max-h-screen overflow-x-auto overflow-y-auto">
                    <table id="itemsTable" class="w-full text-sm text-left text-gray-500 rtl:text-right">
                        <thead class="text-xs text-black uppercase bg-[#00000042]">
                            <tr>
                                <th scope="col" class="px-6 py-3 rounded-tl-lg">IMEI</th>
                                <th scope="col" class="px-6 py-3">Item Name</th>
                                <th scope="col" class="px-6 py-3">Price</th>
                                <th scope="col" class="px-6 py-3">Color</th>
                                <th scope="col" class="px-6 py-3">Storage</th>
                                <th scope="col" class="px-6 py-3">RAM</th>
                                <th scope="col" class="px-6 py-3">Added Date</th>
                                <th scope="col" class="px-6 py-3">Status</th>
                                <th scope="col" class="px-6 py-3 rounded-tr-lg"></th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($imeis as $imei)
                            <tr class="text-black bg-white border-2">
                                <td scope="row"
                                    class="px-6 py-4 font-medium whitespace-nowrap max-2xl:max-w-[100px] truncate cursor-pointer"
                                    title="{{ $imei->imei_number }}">
                                    {{ $imei->imei_number }}
                                </td>
                                <td scope="row"
                                    class="px-6 py-4 font-medium whitespace-nowrap max-2xl:max-w-[100px] truncate cursor-pointer"
                                    title="{{ $imei->mobileItem->name }}">
                                    {{ $imei->mobileItem->name }}
                                </td>
                                <td class="px-6 py-4">
                                    <input type="hidden" name="item-{{ $imei->id }}-single-price" id="item-{{ $imei->id }}-single-price"
                                        value="{{ $imei->mobileItem->mrp_price }}">
                                    <p id="item-{{ $imei->id }}-price">{{ $imei->mobileItem->mrp_price }}</p>
                                </td>
                                <td class="px-6 py-4">
                                    {{ $imei->mobileItem->color->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $imei->mobileItem->storage->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $imei->mobileItem->ram->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $imei->mobileItem->created_at->format('d.m.Y') ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4" id="item-{{ $imei->id }}-subtotal">
                                    {{ $imei->purchaseStatus->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4">
                                <button class="p-3 text-white bg-green-600 border-2 rounded-lg" onclick="navigateToCustomer({{ $imei->id }})">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                        fill="currentColor" class="bi bi-credit-card-fill" viewBox="0 0 16 16">
                                        <path
                                            d="M0 4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v1H0zm0 3v5a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7zm3 2h1a1 1 0 0 1 1 1v1a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1v-1a1 1 0 0 1 1-1" />
                                    </svg>
                                </button>
                                    <button class="hidden p-3 text-white bg-red-600 border-2 rounded-lg">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                            fill="currentColor" class="bi bi-trash-fill" viewBox="0 0 16 16">
                                            <path
                                                d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5M8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5m3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0" />
                                        </svg>
                                    </button>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>

                    <!-- Pagination Links -->




                <div class="flex justify-center p-4 mt-4">
                    @if ($imeis->lastPage() > 1)
                        <ul class="flex flex-wrap gap-2 list-none">
                            <!-- Previous Button -->
                            @if ($imeis->onFirstPage())
                                <li class="px-3 py-2 text-sm bg-gray-300 rounded cursor-not-allowed sm:text-base">
                                    Previous
                                </li>
                            @else
                                <li class="px-3 py-2 text-sm bg-gray-200 rounded hover:bg-gray-400 sm:text-base">
                                    <a href="{{ $imeis->previousPageUrl() }}" class="block">Previous</a>
                                </li>
                            @endif

                            <!-- Page Numbers -->
                            @foreach(range(1, $imeis->lastPage()) as $i)
                                @if ($i == $imeis->currentPage())
                                    <li class="px-3 py-2 text-sm text-white bg-[#1b4f72] rounded sm:text-base">
                                        {{ $i }}
                                    </li>
                                @elseif ($i == 1 || $i == $imeis->lastPage() || abs($i - $imeis->currentPage()) <= 2)
                                    <li class="px-3 py-2 text-sm bg-gray-200 rounded hover:bg-gray-400 sm:text-base">
                                        <a href="{{ $imeis->url($i) }}" class="block">{{ $i }}</a>
                                    </li>
                                @elseif ($i == 2 || $i == $imeis->lastPage() - 1)
                                    <li class="px-3 py-2 text-gray-500">...</li>
                                @endif
                            @endforeach

                            <!-- Next Button -->
                            @if ($imeis->hasMorePages())
                                <li class="px-3 py-2 text-sm bg-gray-200 rounded hover:bg-gray-400 sm:text-base">
                                    <a href="{{ $imeis->nextPageUrl() }}" class="block">Next</a>
                                </li>
                            @else
                                <li class="px-3 py-2 text-sm bg-gray-300 rounded cursor-not-allowed sm:text-base">
                                    Next
                                </li>
                            @endif
                        </ul>
                    @endif
                </div>

                </div>
                <!--white control panel 
                <div class="bg-[#0000000F] h-[200px] max-xl:h-fit flex flex-col rounded-b-lg">
                   
                    <span
                        class="flex gap-3 h-fit w-full justify-evenly py-3 border-[#00000096] border-t-2 border-b-2 max-md:text-sm max-sm:text-xs max-md:flex-col max-md:p-2">
                        <p>Quantity: <span id="wc_qty"></span></p>
                        <p>Total Amount: <span id="wc_t_amount"></span></p>
                        <p>Grand Total: <span id="wc_g_total"></span></p>
                    </span>
                    
                    <div class="flex items-center max-lg:flex-col">
                        
                        <div class="flex flex-col w-1/2 gap-2 p-2 max-lg:w-full">
                            <div class="flex items-center justify-between">
                                <label for="discount"
                                    class="block text-sm font-medium text-gray-900 lg:w-full text-end lg:pr-2 max-sm:text-xs">Discount</label>
                                <input type="text" id="discount"
                                    class="block p-2 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 w-fit"
                                    placeholder="0" required />
                            </div>
                            <div class="flex items-center justify-between">
                                <label for="tax"
                                    class="block text-sm font-medium text-gray-900 lg:w-full text-end lg:pr-2 max-sm:text-xs">Tax</label>
                                <input type="text" id="tax"
                                    class="block p-2 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 w-fit"
                                    placeholder="0" required />
                            </div>
                        </div>
                        
                        <div class="grid w-1/2 grid-flow-col grid-cols-2 grid-rows-2 gap-4 p-2 max-lg:w-full">
                            <button type="button" data-modal-target="payment-modal" data-modal-toggle="payment-modal"
                                onclick="getDataForPaymentModal();"
                                class="row-span-2 gap-2 flex justify-center items-center text-white bg-[#1b4f72] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 max-sm:px-2 py-2 max-sm:py-1 text-center inline-flex items-center max-sm:text-xs">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                    class="bi bi-cash" viewBox="0 0 16 16">
                                    <path d="M8 10a2 2 0 1 0 0-4 2 2 0 0 0 0 4" />
                                    <path
                                        d="M0 4a1 1 0 0 1 1-1h14a1 1 0 0 1 1 1v8a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1zm3 0a2 2 0 0 1-2 2v4a2 2 0 0 1 2 2h10a2 2 0 0 1 2-2V6a2 2 0 0 1-2-2z" />
                                </svg>
                                Pay All
                            </button>
                            <button type="button"
                                class="flex inline-flex items-center justify-center h-full col-span-2 gap-2 px-5 py-2 text-sm font-medium text-center text-white bg-black rounded-lg focus:ring-4 focus:outline-none focus:ring-blue-300 max-sm:px-2 max-sm:py-1 max-sm:text-xs">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                    class="bi bi-exclamation-triangle-fill" viewBox="0 0 16 16">
                                    <path
                                        d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5m.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2" />
                                </svg>
                                Hold All
                            </button>
                            <button type="button"
                                class="flex inline-flex items-center justify-center h-full col-span-2 gap-2 px-5 py-2 text-sm font-medium text-center text-white bg-red-600 rounded-lg focus:ring-4 focus:outline-none focus:ring-blue-300 max-sm:px-2 max-sm:py-1 max-sm:text-xs">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                    class="bi bi-slash-circle" viewBox="0 0 16 16">
                                    <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16" />
                                    <path
                                        d="M11.354 4.646a.5.5 0 0 0-.708 0l-6 6a.5.5 0 0 0 .708.708l6-6a.5.5 0 0 0 0-.708" />
                                </svg>
                                Cancel
                            </button>
                        </div>
                    </div>
                </div>
            -->
            </div>
        </div>
    </div>
    <!-- Add customer Modal -->
    <div id="customer-modal" tabindex="-1" aria-hidden="true"
        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative w-full max-w-2xl max-h-full p-4">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow">
                <!-- Modal header -->
                <div class="flex items-center justify-between p-4 border-b rounded-t md:p-5">
                    <h3 class="text-xl font-semibold text-gray-900">
                        Add Customer
                    </h3>
                    <button type="button"
                        class="inline-flex items-center justify-center w-8 h-8 text-sm text-gray-400 bg-transparent rounded-lg hover:bg-gray-200 hover:text-gray-900 ms-auto"
                        data-modal-hide="customer-modal">
                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 14 14">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                        </svg>
                        <span class="sr-only">Close modal</span>
                    </button>
                </div>
                <!-- Modal body -->
                <div class="p-4 space-y-4 md:p-5">
                    <div class="grid grid-cols-3 gap-6 max-md:grid-cols-1">
                        <div class="max-sm:w-full">
                            <label for="customer_name" class="block mb-2 text-sm font-medium text-black ">Customer
                                Name</label>
                            <input type="text" id="customer_name"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="Enter Customer name" required />
                        </div>
                        <div class="max-sm:w-full">
                            <label for="mobile" class="block mb-2 text-sm font-medium text-black ">Mobile</label>
                            <input type="text" id="mobile"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="Enter mobile" required />
                        </div>
                        <div class="max-sm:w-full">
                            <label for="phone" class="block mb-2 text-sm font-medium text-black ">Phone</label>
                            <input type="text" id="phone"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="Enter phone" required />
                        </div>
                        <div class="max-sm:w-full">
                            <label for="email" class="block mb-2 text-sm font-medium text-black ">Email</label>
                            <input type="email" id="email"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="Enter email" required />
                        </div>
                        <div class="max-sm:w-full">
                            <label for="prev_due" class="block mb-2 text-sm font-medium text-black ">Previous
                                Due</label>
                            <input type="text" id="prev_due"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="Enter previous due" required />
                        </div>
                        <div class="max-sm:w-full">
                            <label for="gst" class="block mb-2 text-sm font-medium text-black ">GST Number</label>
                            <input type="text" id="gst"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="Enter GST number" required />
                        </div>
                        <div class="max-sm:w-full">
                            <label for="tax_no" class="block mb-2 text-sm font-medium text-black ">Tax Number</label>
                            <input type="text" id="tax_no"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="Enter tax number" required />
                        </div>
                        <div class="max-sm:w-full">
                            <label for="province" class="block mb-2 text-sm font-medium text-black ">Province</label>
                            <select id="province"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                <option selected>Choose a province</option>
                                <option value="west">Western Province</option>
                                <option value="east">Eastern Province</option>
                                <option value="south">Southern Province</option>
                                <option value="north">Northern Province</option>
                            </select>
                        </div>
                        <div class="max-sm:w-full">
                            <label for="district" class="block mb-2 text-sm font-medium text-black ">District</label>
                            <select id="district"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                <option selected>Choose a district</option>
                                <option value="gm">Gampaha</option>
                                <option value="col">Colombo</option>
                                <option value="kan">Kandy</option>
                                <option value="kur">Kurunegala</option>
                            </select>
                        </div>
                        <div class="max-sm:w-full">
                            <label for="city" class="block mb-2 text-sm font-medium text-black ">City</label>
                            <!--custom combobox-->
                            <div class="w-full custom-select">
                                <select id="city" name="city"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 hidden">
                                    <option value="">Select city</option>
                                    <option value="City 1">City 1</option>
                                    <option value="City 2">City 2</option>
                                    <option value="City 3">City 3</option>
                                    <option value="City 4">City 4</option>
                                </select>
                            </div>
                        </div>
                        <div class="max-sm:w-full">
                            <label for="p_code" class="block mb-2 text-sm font-medium text-black">Postal
                                Code</label>
                            <!--custom combobox-->
                            <div class="w-full custom-select">
                                <select id="p_code" name="p_code"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 hidden">
                                    <option value="">Select postal code</option>
                                    <option value="1001">1001</option>
                                    <option value="1002">1002</option>
                                    <option value="1003">1003</option>
                                    <option value="1004">1004</option>
                                </select>
                            </div>
                        </div>
                        <div class="max-sm:w-full">
                            <label for="address" class="block mb-2 text-sm font-medium text-black">Address</label>
                            <textarea id="address" rows="2"
                                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                                placeholder="Enter address"></textarea>
                        </div>
                    </div>
                </div>
                <!-- Modal footer -->
                <div class="flex items-center justify-end p-4 border-t border-gray-200 rounded-b md:p-5">
                    <button data-modal-hide="customer-modal" type="button"
                        class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Save</button>
                    <button data-modal-hide="customer-modal" type="button"
                        class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100">Cancel</button>
                </div>
            </div>
        </div>
    </div>
</body>

<script src="https://cdn.jsdelivr.net/npm/flowbite@2.5.2/dist/flowbite.min.js"></script>
<script src="../../../scripts/common.js"></script>
<script>
    function collectBillingData() {
        // Get the table element by its ID
        const table = document.getElementById("itemsTable");

        // Get all rows inside the tbody
        let rows = table.querySelectorAll("tbody tr");

        // Iterate through each row
        rows.forEach(row => {
            // Initialize an array for the current row
            let rowData = [];

            // Get all relevant cells in the current row (excluding the button column)
            let cells = row.querySelectorAll("td, th");

            // Extract text or value from each cell
            cells.forEach(cell => {
                // Check if the cell contains an input element
                let input = cell.querySelector("input");
                if (input) {
                    // Add the value of the input to the rowData array
                    rowData.push(input.value);
                } else if (cell.id && cell.id.includes("subtotal")) {
                    // For subtotal, take the inner text
                    rowData.push(cell.textContent.trim());
                } else {
                    // Add the text content of the cell to the rowData array
                    rowData.push(cell.textContent.trim());
                }
            });

            // Add the row data to the allData array
            allData.push(rowData);
        });

        // Return or log the collected data (ex: ['0: Name', '1: QTY', '2: Price of a Single Unit', '3: Discount Given', '4: Subtotal', ''])
        console.log(allData);
        return allData;
    }

    function getDataForPaymentModal() {
        allData = [];
        let modalAllData = collectBillingData(); // Assuming this is your renamed calculatePayments method
        let t_products = 0; // Total number of products
        let t_amount = 0; // Total amount (sum of all prices)
        let discount = 0; // Total discount
        let tax = document.getElementById("tax").value; // Tax input in the white control panel
        let g_total = 0; // Grand total (sum of all subtotals)

        modalAllData.forEach(rowData => {
            // Convert price, discount, and subtotal to numbers and add them to respective totals
            // (ex: ['0: Name', '1: QTY', '2: Price of a Single Unit', '3: Discount Given', '4: Subtotal of a Row', ''])
            t_products += parseFloat(rowData[1]) || 0;
            t_amount += parseFloat(rowData[1] * rowData[2]) || 0; // Price
            discount += parseFloat(rowData[3]) || 0; // Discount
            g_total += parseFloat(rowData[4]) || 0; // Grand Total without tax
        });

        g_total += parseFloat(tax); // Grand Total with tax

        // Update the modal with the calculated values
        document.getElementById("t_products").innerText = t_products.toString();
        document.getElementById("t_amount").innerText = "Rs. " + t_amount.toFixed(2); // Show 2 decimal places
        document.getElementById("dis").innerText = "Rs. " + discount.toFixed(2); // Show 2 decimal places
        document.getElementById("_tax").innerText = "Rs. " + tax; // tax from white control panel
        document.getElementById("t_type").innerText = "-"; // not yet defined
        document.getElementById("g_total").innerText = "Rs. " + g_total.toFixed(2); // Show 2 decimal places

        // Update the white control panel with the calculated values
        document.getElementById("wc_qty").innerText = t_products.toString();
        document.getElementById("wc_t_amount").innerText = "Rs. " + t_amount.toFixed(2);
        document.getElementById("wc_g_total").innerText = "Rs. " + g_total.toFixed(2);
        document.getElementById("discount").value = discount.toFixed(2);
    }

</script>

</html>


<script>
    function navigateToCustomer(imeiId) {
        window.location.href = `/mobile/mobileCustomer/${imeiId}`;
    }
</script>