...
<div class="h-5/6 max-lg:h-[83vh] flex flex-col">
        ...
        <form action="{{ route('viewotherItem') }}" method="GET" class="flex items-center w-full gap-3">
            <label for="search_cat">Search</label>
            <input type="text" id="search_cat" name="search_cat" value="{{ request('search_cat') }}"
                class="block w-full p-2 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500"
                placeholder="Enter Item name" />
            <button type="submit" class="py-2 px-4 bg-[{{ $settings[7]->value}}] text-white rounded-lg">Search</button>
        </form>
        </div>


        <!--btn controls-->
        <div class="flex items-center justify-between hidden w-full gap-3 px-12 py-5 max-sm:px-6 max-md:flex-col">
                ...
                <tbody>
                    @foreach($mobileItems as $mobileItem)
                        <tr class="text-black bg-white border-2">
                            ...
                            <td>
                                @if(!empty($mobileItem->image_path) && file_exists( $mobileItem->image_path))
                                    <img src="{{ asset('' . $mobileItem->image_path) }}" 
                                        style="width: 40px; height:40px; border-radius:50px;">
                                @else
                                    <img src="{{ asset('upload/item/default.png') }}" 
                                        style="width: 40px; height:40px; border-radius:50px;">
                                @endif
                            </td>

                            <td class="px-6 py-4">{{ $mobileItem->name }}</td>
                            <td class="px-6 py-4">{{ $mobileItem->brand->name }}</td>
                            <td class="px-6 py-4">{{ $mobileItem->category->name }}</td>
                            <td class="px-6 py-4">{{ $mobileItem->serial_numbers_count }}</td> 
                            <td class="px-6 py-4">{{ $mobileItem->retail_price }}</td>
                            ...
                            <td class="flex gap-3 px-6 py-4">
                                <button class="hidden p-3 border-2 rounded-lg" onclick="showImeiModal({{ $mobileItem->id }});">
                                    Add Code
                                </button>
                                ...
                                <button class="p-3 border-2 rounded-lg" onclick="window.location.href='{{ route('Oitem.edit', $mobileItem->id) }}'">
                                    Edit
                                </button>
                            </td>
                        </tr>
    ...
    @if ($mobileItems->lastPage() > 1)
        <ul class="flex flex-wrap gap-2 list-none">
            <!-- Previous Button -->
            @if ($mobileItems->onFirstPage())
            ...
            @else
                <li class="px-3 py-2 text-sm bg-gray-200 rounded hover:bg-gray-400 sm:text-base">
                    <a href="{{ $mobileItems->previousPageUrl() }}" class="block">Previous</a>
                </li>
            @endif

            <!-- Page Numbers -->
            @foreach(range(1, $mobileItems->lastPage()) as $i)
                @if ($i == $mobileItems->currentPage())
                    <li class="px-3 py-2 text-sm text-white bg-[{{ $settings[7]->value}}] rounded sm:text-base">
                        {{ $i }}
                    </li>
                @elseif ($i == 1 || $i == $mobileItems->lastPage() || abs($i - $mobileItems->currentPage()) <= 2)
                    <li class="px-3 py-2 text-sm bg-gray-200 rounded hover:bg-gray-400 sm:text-base">
                        <a href="{{ $mobileItems->url($i) }}" class="block">{{ $i }}</a>
                    </li>
                @elseif ($i == 2 || $i == $mobileItems->lastPage() - 1)
                    <li class="px-3 py-2 text-gray-500">...</li>
                @endif
            @endforeach

            <!-- Next Button -->
            @if ($mobileItems->hasMorePages())
                <li class="px-3 py-2 text-sm bg-gray-200 rounded hover:bg-gray-400 sm:text-base">
                    <a href="{{ $mobileItems->nextPageUrl() }}" class="block">Next</a>
                </li>
            @else
                <li class="px-3 py-2 text-sm bg-gray-300 rounded cursor-not-allowed sm:text-base">
                    Next
    ...
    <div class="relative w-full max-w-2xl max-h-full p-4">
        <div class="relative bg-white rounded-lg shadow-lg">
            <div class="flex items-center justify-between p-4 border-b rounded-t md:p-5">
                <h3 class="text-xl font-semibold text-gray-900">Add Code</h3>
                <button type="button" class="inline-flex items-center justify-center w-8 h-8 text-sm text-gray-400 bg-transparent rounded-lg hover:bg-gray-200 hover:text-gray-900 ms-auto" onclick="closeImeiModal()">
                    ...
                    <span class="sr-only">Close Code List</span>
                </button>
            </div>
...
<script>
    function showModal(whichModal) {
        switch (whichModal) {
            case "imei":
                document.getElementById("modal-header").innerText = "Add Code";
                document.getElementById("modal-input").placeholder = "Code";
                break;
...
<script>
    document.querySelectorAll('.status-toggle').forEach(button => {
        button.addEventListener('click', function () {
            ...
            fetch('{{ route('Oitem.updateStatus') }}', {
                method: 'POST',
                headers: {
