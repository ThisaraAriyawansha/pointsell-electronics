        ...
        <div class="p-6">
            <div class="flex flex-col h-full p-6 border-2 rounded-lg">
                ...
                <div>
                    ...
                    <input type="hidden" id="item_id" name="item_id" value="{{ $RItem->id }}">
                        ...
                        <div>
                            ...
                            <input type="text" id="item_name" name="name"
                                ...
                                placeholder="Your Item name" value="{{ old('name', $RItem->name) }}" required />
                        </div>
                        <div>
                            ...
                            <div class="flex w-full gap-3 custom-select">
                                <select id="brand" name="brand"
                                    ...
                                    <option value="{{ $brand->id }}" {{ $RItem->otherItem_brand_id == $brand->id ? 'selected' : '' }}>{{ $brand->name }}</option> 
                                     @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <div class="flex w-full gap-3 custom-select">
                                <select id="category" name="category"
                                    ...
                                    <option value="{{ $categorie->id }}" {{ $RItem->otherItem_category_id == $categorie->id ? 'selected' : '' }}>{{ $categorie->name }}</option>                                    
                                    @endforeach
                                </select>
                                <button class="py-3 px-6 bg-[{{ $settings[7]->value}}] text-white rounded-lg"
                                    data-modal-target="modal-for-add-items" data-modal-toggle="modal-for-add-items"
                        ...
                        <div>
                            ...
                            <div class="flex w-full gap-3 custom-select">
                                <select id="type" name="type"
                                    ...
                                    <option value="{{ $type->id }}" {{ $RItem->otherItem_type_id == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>                                    
                        ...
                        <div>
                            ...
                            <div class="flex gap-3">
                                <input type="number" id="purchase_price" name="purchase_price"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
                                    placeholder="Enter Price" required min="1" value="{{ old('purchase_price', $RItem->purchase_price) }}" />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex gap-3">
                                <input type="number" id="retails_price" name="retails_price"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
                                    placeholder="Enter Price" required min="1" value="{{ old('retail_price', $RItem->retail_price) }}"/>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex gap-3">
                                <input type="number" id="wholesale_price" name="wholesale_price"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
                                    placeholder="Enter Price" required min="1" value="{{ old('wholesale_price', $RItem->wholesale_price) }}"/>
                            </div>
                        </div>
            ...
            <div class="mb-12 form-group">
                ...
                <a href="#" data-bs-toggle="modal" data-bs-target="#imageModal">
                    <!-- Thumbnail Image -->
                    <img src="{{ asset($RItem->image_path) }}" alt="product Image" class="object-cover w-24 h-24 rounded-lg img-thumbnail">
                </a>
            </div>
                </div>
                <hr>
                ...
                <div class="flex items-center justify-center w-full gap-4 p-4">
                        <button id="saveMobileItemBtn" class="py-3 px-6 bg-[{{ $settings[7]->value}}] text-white rounded-lg" type="submit">
                            <span id="save-btn-text">Update</span> <!-- Text inside the button -->
...
<div id="imageModal" class="fixed inset-0 flex items-center justify-center hidden bg-gray-500 bg-opacity-50">
        <div class="relative w-full max-w-3xl p-4 bg-white rounded-lg">
            ...
            <!-- Full-size Image -->
            <img src="{{ asset($RItem->image_path) }}" alt="Full-size product Image" class="w-full h-auto rounded-lg">
        </div>
    </div>
...
// Add Unit Code (with AJAX)
function addUnitCode() {
    const unitCodeInput = document.getElementById("unit-code-input");
    const unitCode = unitCodeInput.value.trim();
    const itemId = document.getElementById('item_id').value;  // Get the item_id from the hidden input
    
    if (unitCode && itemId) {
        // Make an AJAX request to store the unit code in the database
        fetch('/serial-Number', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                code: unitCode,
                returnable_item_id: itemId,  // Pass the item_id here
            })
        })
        .then(response => response.json())
        .then(data => {
            unitCodeInput.value = "";
            loadUnitCodeList();  // Refresh the unit code list
        })
        .catch(error => {
            console.error('Error:', error);
        });
...
// Load Unit Code list from database and update the quantity
function loadUnitCodeList() {
    const itemId = document.getElementById('item_id').value;  // Get the item_id from the hidden input
    
    // Make an AJAX request to fetch unit codes for the specific item
    fetch(`/serial-Number?other_item_id=${itemId}`)
        .then(response => response.json())
        .then(data => {
            const unitCodeListElement = document.getElementById("unit-code-table-body");
            unitCodeListElement.innerHTML = "";  // Clear current list

            // Loop through the data and create table rows for each unit code
            data.forEach((unitCodeObj, index) => {
                const tr = document.createElement("tr");
                tr.innerHTML = `
                    <td class="px-6 py-3">${unitCodeObj.serial_number}</td>
                    <td class="px-6 py-3">
                        <button class="text-red-500" onclick="deleteUnitCode(${unitCodeObj.id})">Delete</button>
                    </td>
                `;
                unitCodeListElement.appendChild(tr);
            });

            // Update the unit code quantity after loading the list
            updateUnitCodeQty(data.length);
        })
        .catch(error => {
            console.error('Error:', error);
        });
...
// Delete Unit Code (with AJAX)
function deleteUnitCode(id) {
    fetch(`/serial-Number/${id}`, {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
        },
    ...
    .then(response => response.json())
    .then(data => {
        loadUnitCodeList();  // Refresh the unit code list after deletion
    })
    .catch(error => {
        console.error('Error:', error);
    });
...
function updateUnitCodeQty(quantity) {
    document.getElementById("unit-code-qty").innerText = `Qty: ${quantity}`;
}

// Initialize the unit code list on page load
window.onload = function() {
    loadUnitCodeList();  // Load unit code list on page load to show current count
};
...
document.getElementById("saveMobileItemBtn").addEventListener("click", function(event) {
    ...
    // Prepare the form data object for the update
    ...
    formData.append('_method', 'PUT'); // Important for Laravel to recognize this as PUT
    ...
    // Make the AJAX request to update the item
    fetch(`/oItem_edit/${itemId}/edit`, {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            showNotification("Error updating item: " + (data.message || "Unknown error"), "red");
        }
    })
    .catch(error => {
        // Hide loading state
        ...
        showNotification("An error occurred while updating the item. Please try again.", "red");
    });
});
