                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <p class="text-sm font-medium text-gray-700 ms-1 md:ms-2">Import Items</p>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <table id="impTable" class="w-full text-sm text-left text-gray-500 rtl:text-right">
                    <thead class="text-xs text-white uppercase bg-[{{ $settings[7]->value}}]">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3">Min QTY</th>
                            <th scope="col" class="px-6 py-3">Perchase Price</th>
                            <th scope="col" class="px-6 py-3">Retails Price</th>
...
<script>
            ...
            complete: function (results) {
                ...
                rows.forEach((row, index) => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td class="px-6 py-4">${row['Item Code'] || ''}</td>
                        <td class="px-6 py-4">${row['Item Name'] || ''}</td>
                        <td class="px-6 py-4">${row['Qty'] || ''}</td>
                        <td class="px-6 py-4">${row['Supplier ID'] || ''}</td>
                        <td class="px-6 py-4">${row['Min qty'] || ''}</td>
                        <td class="px-6 py-4">${row['Purchase price'] || ''}</td>
                        <td class="px-6 py-4">${row['retail price'] || ''}</td>
                        <td class="px-6 py-4">${row['wholesale price'] || ''}</td>
    ...
    document.getElementById('addDataBtn').addEventListener('click', () => {
        ...
        rows.forEach(row => {
            ...
            const item = {
                item_code: columns[1].textContent.trim(),
                item_name: columns[2].textContent.trim(),
                quantity: columns[3].textContent.trim(),
                supplier_id: columns[4].textContent.trim(),
                min_qty: columns[5].textContent.trim(),
                purchase_price: columns[6].textContent.trim(),
                retail_price: columns[7].textContent.trim(),
                wholesale_price: columns[8].textContent.trim(),
                status_id: columns[9].textContent.trim(),
            };
            itemData.push(item);
        });
        ...
        if (itemData.length > 0) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert('There was an error while adding the items.');
                }
            })
            .catch(error => {
                alert('An error occurred while adding the items.');
                console.error(error);
            });
        } else {
